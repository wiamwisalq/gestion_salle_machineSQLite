package com.example.sqlitesallesmachines.service;
...
import com.example.sqlitesallesmachines.dao.IDao;
import com.example.sqlitesallesmachines.util.MySQLiteHelper;
...
public class SalleService implements IDao<Salle> {
    ...
    public List<ResultatStatique> getStatiqueRes(){
        List<ResultatStatique> reses=new ArrayList<>();
        int machinesCount=0;
        String sql="select idSalle ,count(*) from "+TABLE_MACHINE+" group by idSalle";
        Cursor cursor=this.helper.getReadableDatabase().rawQuery(sql,null);
        ResultatStatique res=null;
        if(cursor.getCount()>0){
            cursor.moveToFirst();
            while (!cursor.isAfterLast()){
                res=new ResultatStatique(cursor.getInt(0),cursor.getInt(1));
                reses.add(res);
                cursor.moveToNext();
            }
